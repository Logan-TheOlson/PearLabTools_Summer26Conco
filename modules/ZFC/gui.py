...
class ZFCModule(tk.Frame):
    ...
    def _build_controls(self):
        ...
        # Mass card (left) and Convert button (right) on same row
    ...
    def _style_ax(self, ax):
        ...
        ax.set_title("ZFC / FC Magnetization vs Temperature", color=TEXT_DIM, fontsize=8.5, pad=6)
    ...
    def _draw_preview(self, df):
        ...
        self._ax.plot(x, df["Magnetization ZFC (A m^2/kg)"],
                      color=COLOR_ZFC, linewidth=1.2, zorder=1)
        self._ax.scatter(x, df["Magnetization ZFC (A m^2/kg)"],
                         color=COLOR_ZFC, s=18, zorder=2, linewidths=0, label="ZFC")

        self._ax.plot(x, df["Magnetization FC (A m^2/kg)"],
                      color=COLOR_FC, linewidth=1.2, zorder=1)
        self._ax.scatter(x, df["Magnetization FC (A m^2/kg)"],
                         color=COLOR_FC, s=18, zorder=2, linewidths=0, label="FC")

        self._ax.legend(fontsize=7.5, facecolor=SURFACE, edgecolor=BORDER, labelcolor=TEXT)
        self._canvas.draw()
